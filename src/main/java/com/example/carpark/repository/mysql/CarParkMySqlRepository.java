...
public interface CarParkMySqlRepository extends JpaRepository<CarPark, Long> {
     ...
     */
    @Query(
        "SELECT cp FROM CarPark cp WHERE cp.availableLots > 0 AND cp.deletedAt IS NULL"
    )
     ...
     */
    @Query(
        "UPDATE CarPark cp SET cp.deletedAt = NOW(), cp.updatedAt = NOW(), cp.updatedBy = :updatedBy WHERE cp.id = :id"
    )
    @Modifying
    @Transactional
    int softDeleteById(
     ...
     */
    @Query(
        "UPDATE CarPark cp SET cp.deletedAt = NULL, cp.updatedAt = NOW(), cp.updatedBy = :updatedBy WHERE cp.id = :id"
    )
    @Modifying
    @Transactional
     ...
     */
    @Query(
        "SELECT cp FROM CarPark cp WHERE cp.carParkType = :carParkType AND cp.deletedAt IS NULL"
    )
     ...
     */
    @Query(
        "SELECT cp FROM CarPark cp WHERE cp.shortTermParking = :shortTermParking AND cp.deletedAt IS NULL"
    )
     ...
     */
    @Query(
        "SELECT COUNT(cp) FROM CarPark cp WHERE cp.availableLots > 0 AND cp.deletedAt IS NULL"
    )
